...
function getFractionalPart(num) {
  ...
  const firstfractionalPart = num - Math.floor(num);
  const fractionalPart = Math.fround(firstfractionalPart);
...
function compareAndRound(num1, num2) {
  ...
  const roundNum = Math.round(maxNum);
...
function estimateInvestment(principal, interestRate, years) {
  if (
    ...
    console.log("Помилка. Aргументи мають бути числами");
    return null;
  }

  const sumOfInvestment = principal * Math.pow(1 + interestRate, years);
  const roundInvestment = Math.round(sumOfInvestment);

  return roundInvestment;
